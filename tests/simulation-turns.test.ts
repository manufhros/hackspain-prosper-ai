...
function socketPeer() {
  ...
  const socket = {
    ...
    close() { if (socket.readyState === WebSocket.CLOSED) return; socket.readyState = WebSocket.CLOSED; socket.onclose?.({ code: 1000 }); },
  };
  return {
    socket, sent,
...
test("audio without optional marks gets a spoken reply and a late mark does not duplicate the turn", async () => {
  ...
  expect(call.wire_stats).toEqual({ received_audio_bytes: 160, received_marks: 1, turns_without_marks: 1 });

...
Run `bun run test:consent:live` for a finite, billable text-only evaluation against Jev. It starts no servers, uses synthetic fixtures, touches no clinic records, and saves results to `.workbench/consent-evaluation-*.json`. Offline `bun run check` tests the adapter and state transitions with mocked decisions. The initial live evaluation passed 23/23 EN/ES/CA fixtures, including informal and emphatic confirmations, cancellation, corrections, conditions and interrupted-offer protection; this is not an end-to-end audio or concurrency benchmark.

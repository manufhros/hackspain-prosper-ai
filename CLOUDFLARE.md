...
The desk's OpenNext build command uses Webpack with `DESK_CLOUDFLARE_BUILD=1`.
This keeps Next's standalone tracing root at `desk/`, matching the package root
OpenNext detects from `desk/package-lock.json`, while compiling the shared log
parser outside that directory. Local Turbopack builds retain the repository root
for shared-source resolution. In Next 16, `turbopack.root` also determines the
standalone tracing root; using the repository root for OpenNext nests the output
under `standalone/desk/` and breaks middleware packaging. Use `npm run cf:check`
from `desk/` to validate the adapter and final Worker bundle, not only `npm run build`.

...
export function rms(pcm: Int16Array) {
...
export function resample(pcm: Int16Array, fromRate: number, toRate: number): Int16Array {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const frac = x - i0;
    out[i] = Math.round((pcm[i0] ?? 0) + ((pcm[i1] ?? 0) - (pcm[i0] ?? 0)) * frac);
  }
  return out;
}
...
function ascii(view: DataView, offset: number, n: number) {
  let s = "";
...
export const SILENCE_FRAME = new Uint8Array(FRAME_SAMPLES).fill(0xff);

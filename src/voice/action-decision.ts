...
export function selectedAction(decision: ActionDecision, candidate?: ActionCandidate): ActionChoice {
  ...
  // Model uncertainty is not evidence that the caller's intent is unclear.
  // Repair the candidate (or continue work after an uncertain finish decision).
  return decision.probability >= minimum ? decision.choice : "revise";
}
export const actionInstructions = [
  ...
  "Keep the requested action distinct from missing identity or scheduling details. If a DNI/NIE is incomplete, ambiguous or rejected, a question asking for the full identifier including its letter is an appropriate next step. Do not require the caller to restate an already clear booking, cancellation or rescheduling request. Reject generic questions about what to do when the conversation already states the action.",
].join("\n");
